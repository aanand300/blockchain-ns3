...
//To print throughput in (step_time) Minutes slot
    int handlecount = 0;
    int slot_itr = 0;
    long cumulative_blockMessageSize = 0;
    std::map<double, std::pair<double, long> > slot_tp; //map<Time, pair<throughput, iterations in that slot> >

//Map for storing each node's throughput
    std::map<int, std::pair<double, long> > nodethroughput_map; //map<NodeID, pair<throughput, NodeID's iteration count> >
    std::vector<std::map<int, std::pair<double, long> > >nodetpvec; //vector of nodethroughput_map

    void
    BitcoinNode::PrintThroughput(double stop, long step_time)
    {
        if(Simulator::Now().GetMinutes()<=stop){
            double phasetp = cumulative_blockMessageSize/(step_time * 60); // (throughput/second)
            std::cout<<"Throughput at "<<Simulator::Now().GetMinutes() <<" mins is: "<<phasetp<<" bytes/sec\n";
            slot_tp.insert(std::pair<double , std::pair<double, long> >(Simulator::Now().GetMinutes(), std::make_pair(phasetp, slot_itr)));
            nodetpvec.push_back(nodethroughput_map); //pushing map of nodethroughput_map created to a vector
            cumulative_blockMessageSize = 0;
            slot_itr = 0;
            nodethroughput_map.clear();
            Simulator::Schedule(Minutes(step_time), &BitcoinNode::PrintThroughput, stop, step_time);
        }
    }

//Print Node-wise throughput
    void
    BitcoinNode::PrintNode_SlotThroughput(long step_time, int tpfidx) {

        std::ofstream ftp;
        std::string progidx = std::to_string(tpfidx);
        std::string ftppath = "/home/ns3/Desktop/ns-allinone-3.25/ns-3.25/results_current/miner_expt/overalltp-rd-model/min48/iter"+progidx+".txt";
        ftp.open(ftppath);
        std::cout<<"Throughput at each "<<step_time<<" min slot is:\n";
        for(auto i = slot_tp.begin(); i!=slot_tp.end(); i++){
            double time = (*i).first;
            double throughput = (*i).second.first;
            long iteration = (*i).second.second;
            if(iteration != 0){
                std::cout<<time<<" "<<throughput/iteration<<"\n";
                ftp << time<<" "<<throughput/iteration<<"\n";
            }
            else{
                std::cout<<time<<" 0\n";
                ftp <<time<<" 0\n";
...
//    std::cout<<"Node-wise throughput for each time slot:\n";
//    int vec_cnt=1;
//    int nwfnum = 25;
//    for(auto i = nodetpvec.begin(); i!= nodetpvec.end(); i++){
//
//        std::ofstream fnw;
//        std::string nwidx = std::to_string(nwfnum);
//        std::string nwpath = "/home/ns3/Desktop/ns-allinone-3.25/ns-3.25/results_current/miner_expt/nwtp1/gauss/iter"+progidx+"/time"+nwidx+".txt";
//        fnw.open(nwpath);
//        std::cout<<"For slot: "<<vec_cnt<<" ,throughput: \n";
//        for(auto j = (*i).begin(); j!= (*i).end(); j++){
//            //std::cout<<"Node ID: "<<(*j).first<<" , iteration: "<<(*j).second.second<<" , Throughput: "<<(*j).second.first/((*j).second.second*step_time*60)<<" bytes per iteration per second\n";
//            std::cout<<(*j).first<<" "<<(*j).second.first/((*j).second.second*step_time*60)<<"\n";
//            fnw <<(*j).first<<" "<<(*j).second.first/((*j).second.second*step_time*60)<<"\n";
//        }
//        vec_cnt++;
//        nwfnum += 25;
//        fnw.close();
                ...
                while ((pos = totalReceivedData.find(delimiter)) != std::string::npos)
                {
                    ...
                    switch (d["message"].GetInt())
                    {
                        case INV:
                        {
                            ...
                            //std::cout<<"Node: "<<m_nodeStats->nodeId<<" received INV msg: "<<m_bitcoinMessageHeader + m_countBytes + d["inv"].Size()*m_inventorySizeBytes<<"\n";
                        ...
                        case EXT_INV:
                        {
                            ...
                            std::cout<<"Node: "<<m_nodeStats->nodeId<<" received EXT_INV msg: "<<m_bitcoinMessageHeader + m_countBytes + d["inv"].Size()*m_inventorySizeBytes<<" Bytes\n";
...
/*                     PrintQueueChunks();
                ...
                PrintReceivedChunks(); */
                        ...
                        case GET_HEADERS:
                        {
                            ...
                            //std::cout<<"Node: "<<m_nodeStats->nodeId<<" received GET_HEADERS msg: "<<m_bitcoinMessageHeader + m_getHeadersSizeBytes<<" Bytes\n";
                        ...
                        case EXT_GET_HEADERS:
                        {
                            ...
                            std::cout<<"Node: "<<m_nodeStats->nodeId<<" received EXT_GET_HEADERS msg: "<<m_bitcoinMessageHeader + m_getHeadersSizeBytes<<" Bytes\n";
                        ...
                        case GET_DATA:
                        {
                            ...
                            //std::cout<<"Node: "<<m_nodeStats->nodeId<<" received GET_DATA msg: "<<m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size()*m_inventorySizeBytes<<" Bytes\n";
...
/*                 std::cout << "Node " << GetNode()->GetId() << "-" << InetSocketAddress::ConvertFrom(from).GetIpv4 ()
                      ...
                      << Simulator::Now ().GetSeconds() << "s \n"; */
                        ...
                        case EXT_GET_DATA:
                        {
                            ...
                            std::cout<<"Node: "<<m_nodeStats->nodeId<<" received EXT_GET_DATA msg: "<<m_bitcoinMessageHeader + m_countBytes + d["chunks"].Size()*m_inventorySizeBytes<<" Bytes\n";
...
/*                 std::cout << "Node " << GetNode()->GetId() << "-" << InetSocketAddress::ConvertFrom(from).GetIpv4 ()
                      ...
                      << Simulator::Now ().GetSeconds() << "s \n"; */
                        ...
                        case HEADERS:
                        {
                            ...
                            //std::cout<<"Node: "<<m_nodeStats->nodeId<<" received HEADERS msg: "<<m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size()*m_headersSizeBytes<<" Bytes\n";
                        ...
                        case EXT_HEADERS:
                        {
                            ...
                            std::cout<<"Node: "<<m_nodeStats->nodeId<<" received EXT_HEADERS msg: "<<m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size()*m_headersSizeBytes<<" Bytes\n";
...
/*                     PrintQueueChunks();
                ...
                PrintReceivedChunks(); */
                        ...
                        case BLOCK:
                        {
                            ...
                            handlecount++;
                            slot_itr++;
                            cumulative_blockMessageSize += blockMessageSize;
                            //std::cout<<handlecount<<" "<<cumulative_blockMessageSize<<"\n";
                            m_nodeStats->blockReceivedBytes += blockMessageSize;

                            //put throughput in map
                            if(nodethroughput_map.find(m_nodeStats->nodeId) == nodethroughput_map.end())
                            {
                                nodethroughput_map.insert(std::pair<int, std::pair<double, long> >(m_nodeStats->nodeId, std::make_pair(blockMessageSize, 1)));
                            }
                            else{
                                nodethroughput_map[m_nodeStats->nodeId].first += blockMessageSize;
                                nodethroughput_map[m_nodeStats->nodeId].second ++;
                            }
